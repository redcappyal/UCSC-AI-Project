...
.venv/bin/python -m pytest tests/ -q     # ~722 tests, ~45s
```
...
green run reads "803 passed, 1 skipped, 1 deselected" (measured 2026-07-30). Both are expected, not failures to
...
- **The ball detector default is `rfdetr` (hosted Roboflow RF-DETR), not WASB** — one
  constant, `ball_track_offline.BALL_DETECTOR_DEFAULT`, read only by
  `selected_detector()`, so every entry point agrees. WASB is opt-in via
  `BALL_DETECTOR=local` until the line-call eval scores it; do not add a second place
  that pins a default (app.py used to, and the two disagreed). Because rfdetr is the
  default again, `tracking_common.CONFIDENCE_THRESHOLD` is the live pipeline floor.
- `ball_track_offline.py` (formerly `stereo_offline.py`, renamed when the stereo half moved
  ...
  trained ball detector (`ball_model.py` + `ball_detector.py`), reached with
  `BALL_DETECTOR=local`. It loads `models/crosscourt-wasb-416-v1/` and needs `torch`. That artifact is *committed* (manifest
