...
def make_floor_bounce_rows(camera, fps=60.0):
    """CSV-row dicts for a lob that bounces on the floor mid-court.
    Returns (rows, bounce_frame)."""
    position = np.array([10.0, 26.0, 6.0])
    velocity = np.array([1.0, -18.0, -14.0])
    dt = 1.0 / fps
    rows, bounce_frame = [], None
    for frame in range(40):
        u, v = camera.project(position)
        ...
        velocity_next = velocity + GRAVITY_VEC * dt
        position_next = position + velocity * dt + 0.5 * GRAVITY_VEC * dt * dt
        if position_next[2] <= 0.0 and bounce_frame is None:
            bounce_frame = frame + 1
            position_next[2] = -position_next[2]
            velocity_next[2] = -0.7 * velocity_next[2]
        position, velocity = position_next, velocity_next
    return rows, bounce_frame
